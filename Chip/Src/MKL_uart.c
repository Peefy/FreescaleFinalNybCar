...
void uart_putByte (UARTn_e uratn, uint8 ch)
{
    if(uratn == UART0)
    {
        //等待发送缓冲区空
...
void uart1_test_handler(void)
{
    ...
    {        
        ...
        uart_getchar(UART1,&recieve);//用户需要处理接收数据
        LC_PutByteInFIFObuffer(recieve);
#endif 
#ifdef  NA_Car
        uart_getchar(UART1,&recieve);//用户需要处理接收数据
        LC_PutByteInFIFObuffer(recieve);
        ...
        //Recieve_OC_Mes();                 //牛A收牛B的数据
        uart_getchar(UART1,&recieve);//用户需要处理接收数据
        LC_PutByteInFIFObuffer(recieve);
#endif        
    }
    //led(LED0,LED_ON);   //开机指示灯
    //kaiji_fmq();
